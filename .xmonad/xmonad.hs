...
myBrowser :: String
myBrowser = "floorp "  -- Sets qutebrowser as browser
...
myFocusColor  = color17     -- This variable is imported from Colors.THEME
...
myWorkspaces = ["1: dev ", "2: www ", "3: sys ", "4: doc ", "5: game ", "6: chat ", "7: mus ", "8: vid ", "9: misc "]
-- myWorkspaces =
--         " 1 : <fn=2>\xf111</fn> " :
--         " 2 : <fn=2>\xf1db</fn> " :
--         " 3 : <fn=2>\xf192</fn> " :
...
myManageHook = composeAll
  ...
  , title =? "Ablaze Floorp"                            --> doShift ( myWorkspaces !! 1 )
  , className =? "mpv"                                  --> doShift ( myWorkspaces !! 7 )
  , className =? "Gimp"                                 --> doShift ( myWorkspaces !! 8 )
  -- , className =? "steam_app_252950" --> doFullFloat
  , className =? "steam_app_252950"                     --> doShift  ( myWorkspaces !! 4 )
  , className =? "discord"                              --> doShift ( myWorkspaces !! 5 )
  , (className =? "firefox" <&&> resource =? "Dialog")  --> doFloat  -- Float Firefox Dialog
  -- , className =? "steam"   --> doShift ( myWorkspaces !! 4)
  , title =? "Steam"                                    --> doShift( myWorkspaces !! 4)
  , title =? "Jorgen"                                   --> doShift ( myWorkspaces !! 5)
  , className =? "clickup"                              --> doShift ( myWorkspaces !! 8)
...
myKeys c =
  ...
  , ("M-S-q", addName "Quit XMonad"            $ spawn "pkill X")
  , ("M-q", addName "Kill focused window"    $ kill1)
  , ("M-S-a", addName "Kill all windows on WS" $ killAll)
  , ("M-d", addName "Run prompt"      $ spawn "~/.local/bin/dm-run")
  ...
  -- , ("M-m", addName "Enable Mouse"                             $ spawn "xinput set-prop 16 'Device Enabled' 1")
  -- , ("M-S-m", addName "Disable Mouse"                             $ spawn "xinput set-prop 16 'Device Enabled' 0")
  ...
  ^++^ subKeys "Misc."
  [ ("M-n", addName "Toggle Notification center" $ spawn "kill -s USR1 $(pidof deadd-notification-center)")]
...
main = do
  ...
  xmproc1 <- spawnPipe ("xmobar -x 2 $HOME/.config/xmobar/doom-one-xmobarrc")
  xmproc2 <- spawnPipe ("xmobar -x 1 $HOME/.config/xmobar/doom-one-xmobarrc")
  -- the xmonad, ya know...what the WM is named after!
  xmonad $ addDescrKeys' ((mod4Mask, xK_F1), showKeybindings) myKeys $ ewmh $ docks $ def
    { manageHook         = myManageHook <+> manageDocks
    , handleEventHook    = windowedFullscreenFixEventHook <> swallowEventHook (className =? "Alacritty"  <||> className =? "st-256color" <||> className =? "XTerm" <||> className =? "kitty" <||> className =? "com.mitchellh.ghostty" <||> className =? "contour") (return True) <> trayerPaddingXmobarEventHook
    , modMask            = myModMask
    , terminal           = myTerminal
